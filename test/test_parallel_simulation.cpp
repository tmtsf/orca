...
namespace orca { namespace test {
  ...
  TEST(ParallelSimulation, EuropeanOptionAAD)
  {
    aad::Number spot{100.};
    aad::Number discount{.03};
    aad::Number dividend{.02};
    aad::Number vol{.2};

    auto model = simulation::model::BlackScholesModel<aad::Number>(spot,
                                                                   vol,
                                                                   false,
                                                                   discount,
                                                                   dividend);
    ...
    double expiry = 1.;
    double settlement = 1. + 2./365.;
    auto option = simulation::product::EuropeanOption<aad::Number>(strike, expiry, settlement);
